...
def draw_ball_position(frame, court_detector, xy, i):
  """
  Calculate the ball position using the inverse transformation of the court and the x, y positions
  """
  inv_mats = court_detector.game_warp_matrix[i]

  # Ball locations
  if xy != [None,None]:
    p = np.array(xy,dtype='float64')
    ball_pos = np.array([p[0].item(), p[1].item()]).reshape((1, 1, 2))
    transform = cv2.perspectiveTransform(ball_pos, inv_mats)[0][0].astype('int64')
    cv2.circle(frame, (transform[0], transform[1]), 15, (0,255,255), -1)
    return transform
  else:
    return [None,None]
...
def create_top_view(court_detector, xy, fps):
    """
    ...
    out = cv2.VideoWriter('VideoOutput/minimap.mp4',cv2.VideoWriter_fourcc(*'mp4v'), fps, (v_width, v_height))

    transforms=[]
    for i in range(len(xy)):
        frame=court.copy()
        transform = draw_ball_position(frame, court_detector, xy[i], i)
...
if __name__ == "__main__":
    ...
    # video Writer
    court_video = cv2.VideoWriter('VideoOutput/output1.mp4',fourcc, fps, (v_width, v_height))
    ...
    while True:
        ...
        if ret==False:
            break
        if frame_i == 1: # for the first frame,we use detect() to find table
            print('First Frame')
        ...
        # draw the table line on the frame
        for i in range(0, len(lines), 4):
            x1, y1, x2, y2 = lines[i],lines[i+1], lines[i+2], lines[i+3]
